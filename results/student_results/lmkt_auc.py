...
#These are the IDs of questions in the test set that are not included in the Duolingo training set (not seen ever for any student)
...
def auc_seen(results, unseen_idx):
    seen_idx = [i for i in range(len(results[-1])) if i not in unseen_idx]
    y = np.array(results[7])[seen_idx]
    pred = np.array(results[5])[seen_idx]
    auc = []
    for i in range(100):
        np.random.seed(i)
        bootstrap_idx = np.random.choice(range(len(y)), len(y))
        fpr, tpr, thresholds = metrics.roc_curve(y[bootstrap_idx], pred[bootstrap_idx])
        auc.append(metrics.auc(fpr, tpr))
    print("AUC: "+str(np.mean(auc))+" "+str(np.std(auc)))
    return auc

def auc_unseen(results, unseen_idx):
    print(len(results[-1]))
    y = np.array(results[7])[unseen_idx]
    pred = np.array(results[5])[unseen_idx]
    auc = []
    for i in range(1000):
        np.random.seed(i)
        bootstrap_idx = np.random.choice(range(len(y)), len(y))
...
def auc(results):
    y = np.array(results[7])
    pred = np.array(results[5])
    auc = []
    for i in range(100):
        np.random.seed(i)
        bootstrap_idx = np.random.choice(range(len(y)), len(y))
...
for model_type in ["_lmkt", "_dkt", "_questiononly"]:
	print(model_type[1:])
	print("Spanish")
	dynamic_results = read_file(data_filename="student_models_results/spanish"+model_type+".txt", key_filename="spanish_test_key")
	auc_seen(dynamic_results, spanish_unseen)
	auc_unseen(dynamic_results, spanish_unseen)
	print("\n")
	print("French")
	dynamic_results = read_file(data_filename="student_models_results/french"+model_type+".txt", key_filename="french_test_key")
	auc_seen(dynamic_results, french_unseen)
	auc_unseen(dynamic_results, french_unseen)
	print("\n")
	print("\n")